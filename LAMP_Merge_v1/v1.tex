...
\definecolor{LAMPYellow}{RGB}{255,247,210}
...
\newcommand{\tblhead}[1]{\begingroup\setlength{\fboxsep}{1.2pt}\colorbox{HeaderGray}{\strut #1}\endgroup}
\newcommand{\lampcell}[1]{\begingroup\setlength{\fboxsep}{1.2pt}\colorbox{LAMPYellow}{\strut #1}\endgroup}
\newcommand{\avgcell}[1]{\begingroup\setlength{\fboxsep}{1.2pt}\colorbox{AvgBlue}{\makebox[2.85em][c]{\strut #1}}\endgroup}
\newcommand{\lampband}{\noalign{\begingroup\color{LAMPYellow}\hrule height 2.35ex\endgroup\kern-2.35ex}}
\newcommand{\headband}{\noalign{\begingroup\color{HeaderGray}\hrule height 2.35ex\endgroup\kern-2.35ex}}
\newcommand{\gainup}[1]{\textcolor{GainGreen}{\(\uparrow\)#1}}
\newcommand{\gaindown}[1]{\textcolor{GainGreen}{\(\downarrow\)#1}}
\newcommand{\dropdown}[1]{\textcolor{DropRed}{\(\downarrow\)#1}}
\newcommand{\neutraldelta}[1]{\textcolor{GainGreen}{#1}}
...
Medical artificial intelligence is transitioning from single-center model development toward multicenter collaborative deployment. The reliability of clinical diagnostic models typically depends on experience spanning hospitals, imaging devices, and patient populations, as no single institution can readily cover the full disease spectrum, rare cases, and diverse imaging conditions simultaneously \cite{sheller2020federated,rieke2020future}. However, medical data are jointly constrained by ethical approval requirements, privacy regulations, device heterogeneity, and institutional workflows, making centralization of raw images on a common server difficult\cite{kaissis2020secure}. A more practical collaboration paradigm is for hospitals to train models locally and, upon completion, participate in global model construction through limited information sharing.
...
Federated and decentralized learning perform collaborative training through communication protocols. FedAvg and its variants rely on repeated server-client updates\cite{pmlr-v54-mcmahan17a,kaissis_secure_2020}; Swarm Learning and Gossip Learning reduce centralization but still require participants to continue training, exchanging models, and optimizing from intermediate states\cite{warnat-herresthal_swarm_2021,hegedhus_gossip_2019}. However, our problem is single-shot asynchronous model merging after local training has finished: hospitals may not remain online, receive global checkpoints repeatedly, or run additional optimization rounds. \textbf{The limitation is therefore communication feasibility rather than only privacy.} LAMP-Merge requires one upload of a local model interface and class-level statistics, enabling the server to construct a unified diagnostic model without a multi-round feedback loop.
...
\begin{tabular}{lrrr}
\hline
\tblhead{Metric} & \tblhead{LAMP-Merge} & \tblhead{Best reference} & \tblhead{Difference} \\
\hline
...
% 所有原型均位于共享参考特征空间 z=phi_0(T(x)) 中。我们仅将 t-SNE 用于分析，而不用于服务器端选择或融合。图中给出了 Blood 数据集上的联合 t-SNE 对比，不同消融设置被投影到同一二维空间中，用于观察诊断原型是否与真实类别结构保持一致。
All prototypes live in the shared reference feature space $z=\phi_0(T(x))$. We use t-SNE only for analysis, not for server-side selection or fusion. Fig.~\ref{fig:tsne} shows a joint t-SNE comparison on the Blood dataset, where different ablation settings are projected into the same two-dimensional space to examine whether diagnostic prototypes remain aligned with the true class structure.

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/tsne_joint_bloodmnist_224_resnet.png}
\caption{Joint t-SNE visualization of ResNet feature representations and diagnostic prototypes on the Blood dataset. From left to right: LAMP-Merge, classifier-head aggregation, global-feature mean, and shuffled-label prototype.}
\label{fig:tsne}
\end{figure*}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/full_scope_prototype_geometry_internal_ablation.png}
