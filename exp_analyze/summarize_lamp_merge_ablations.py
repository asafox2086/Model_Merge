...
ABLATION_TABLE = ROOT / "My_merge_ret/LAMP-Merge消融表.md"
...
def iter_main_table_cells(table_path):
    ...
    for (_section, _model, kind), table in tables.items():
        ...
        for row in table["rows"]:
            ...
            if method in {FORMAL_LABEL, "lamp_merge"}:
                mine = row
            else:
...
def render_report(
    ...
    lines.extend([
        ...
        "## 实验来源",
        "",
        "- 正式 LAMP-Merge 全量结果：`outputs/lamp_merge_full`。",
