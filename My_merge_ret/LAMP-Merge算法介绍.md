...
其中 \(\rho\in(0,1)\)，当前实现默认 \(\rho=0.45\)。这个幂次让样本更多的客户端拥有更高证据，但避免大客户端完全垄断某个诊断类别。
...
$$
q_c=\frac{\sum_{i=1}^{K}m_{i,c}}{\sum_{k=1}^{C}\sum_{i=1}^{K}m_{i,k}}.
$$
...
$$
b_c=\lambda\left(\log q_c-\frac{1}{C}\sum_{k=1}^{C}\log q_k\right).
$$
