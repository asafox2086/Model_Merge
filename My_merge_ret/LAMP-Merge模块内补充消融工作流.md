...
| \(n_{i,c}\) | M1 的类别支持数，\(n_{i,c}=|D_{i,c}|\) | 只表示原型可靠性支持数 |
...
| \(e_{i,c}\) | 类别证据权重，\(e_{i,c}=(n_{i,c}+1)^\gamma\mathbf{1}[n_{i,c}>0]\) | 证据指数必须写作 \(\gamma\)，不能写作 \(\rho\) |
| \(\alpha_{i,c}\) | 类别级客户端可靠性权重 | 用于聚合客户端原型 |
| \(p_c\) | 服务端聚合得到的全局诊断原型 | 不写作 \(\hat{\mu}_c\) |
| \(w_c\) | 原型分类头权重，\(w_c=s p_c/\|p_c\|_2\) | 与分类分数对应 |
| \(s\) | prototype head scale | 只表示 M1 尺度 |
| \(\pi_c\) | 由 \(m_{i,c}\) 估计的全局类别先验 | 不写作 \(q_c\) 或 \(\hat{\pi}_c\) |
| \(r\) | 主导类不平衡强度，\(r=C\max_c\pi_c\) | 用于 M2 触发判断 |
| \(\tau\) | M2 长尾触发阈值 | 不表示校准强度 |
| \(\lambda\) | M2 有界长尾校准强度 | 不表示阈值 |
| \(b_c\) | M2 的中心化 log-prior bias | 只表示类别偏置 |
| \(q(c)\) | 测试集预测类别分布 | 只用于预测分布诊断，不表示类别先验 |
| \(\rho\) | collapse ratio，\(\rho=\max_c q(c)\) | 只表示预测坍缩强度，不能表示证据指数 |
| \(C_{\mathrm{eff}}\) | effective predicted classes | 不与类别数 \(C\) 混用 |
