...
$$
\mathcal{Y}_i \subset \mathcal{Y}.
$$

当类别 $c\notin\mathcal{Y}_i$ 时，客户端 $i$ 没有关于类别 $c$ 的监督样本。记客户端模型为 $f_i$，融合模型为 $f_{\mathrm{merge}}$，测试集为

$$
\mathcal{D}_{\mathrm{test}}=\{(x_n,y_n)\}_{n=1}^{N}.
$$

定义任意模型 $f$ 在测试集上的预测类别分布为

$$
q_f(c)=\frac{1}{N}\sum_{n=1}^{N}\mathbf{1}\left[\arg\max_{y\in\mathcal{Y}} f_y(x_n)=c\right].
$$

对应的预测坍缩强度定义为

$$
\rho(f)=\max_{c\in\mathcal{Y}} q_f(c).
$$

我们观测到，在若干医学 case 中，客户端预测仍覆盖多个类别，即

$$
\rho(f_i) < 1
$$

且有效预测类别数大于 1；但通用融合模型满足

$$
\rho(f_{\mathrm{merge}})\approx 1.
$$
...
医学图像分类常对应真实临床流行率或采样流行率，因此常见病、常见器官或高频诊断类别可能占据很大比例。设测试集真实类别分布为

$$
\pi(c)=P(y=c).
$$

如果模型完全坍缩到高频类别 $c^\star$，即对所有样本都输出
