...
**观测 1：客户端没有见过的类别会造成未校准输出，通用融合会把这种未校准性放大成全局预测坍缩。**

我们在医学多中心设置中观察到一个稳定现象：单个客户端通常只见过全局类别空间的一部分，因此它对未见类别没有有效监督，只能依赖随机初始化、预训练偏置或与已见类别的间接相关性给出输出。尽管如此，客户端模型本身并不一定完全坍缩；它们往往仍会在自己熟悉的若干类别之间分配预测。问题发生在融合之后：通用 post-hoc 融合方法直接在参数空间平均、裁剪或符号聚合多个客户端模型，却没有显式记录“哪个客户端可靠学习过哪个类别”。结果是，多个客户端对未见类别的未校准输出被混合进同一个全局模型，最终融合模型在测试集上大量输出同一个类别，表现为预测分布从“多个局部类别”退化为“全局单类输出”。

本文采用两个直接对应图表的诊断量。第一个量是预测类别分布：

$$
q(c)=\frac{1}{N}\sum_{n=1}^{N}\mathbf{1}\left[\hat{y}_n=c\right].
$$

其中，`N` 表示测试图像数量，`c` 表示某个诊断类别，`q(c)` 表示模型把多少比例的测试图像预测成该类别，`\hat{y}_n` 表示第 `n` 张测试图像的预测类别。第二个量是预测坍缩强度：

$$
\rho=\max_c q(c).
$$

`rho` 是预测分布中最大的类别占比。它越接近 1，说明模型越接近“几乎所有图像都预测为同一个类别”。我们观测到，在若干医学 case 中，单个客户端的 `rho` 并不等于 1，并且 effective classes 大于 1，说明客户端仍保留多个局部类别的预测能力；但通用融合之后，融合模型的 `rho` 接近 1，effective classes 接近 1，说明预测已经退化成全局单类输出。

例如在 `organsmnist_224 / resnet / clients=3 / beta=0.01` 中，三个客户端的 effective classes 分别为 1.72、5.00 和 3.83，而 `breadcrumbs` 融合后的 effective classes 只有 1.15，collapse ratio 达到 0.9738。这说明坍缩不是简单来自“客户端已经完全崩溃”，而是通用融合把客户端未见类上的未校准输出进一步放大成了全局单类预测。

**观测 2：医学长尾先验会使单类坍缩在 accuracy 上看起来并不差。**

医学图像分类常对应真实临床流行率或采样流行率，因此常见病、常见器官或高频诊断类别可能占据很大比例。设测试集中最大类别占比为 `p_max`。如果一个模型完全坍缩到这个最高频类别，那么它虽然没有真正的多类别诊断能力，但仍可以得到如下 accuracy：

$$
\mathrm{Acc}_{\mathrm{collapse}}=p_{\max}.
$$

因此，当最高频类别占比很大时，一个没有多类别诊断能力的坍缩模型也可能获得看似较高的 accuracy。我们在结果中确实观察到，部分基线方法虽然 collapse ratio 很高、effective classes 很低，但在长尾数据集上仍能取得较高 accuracy。这说明医学模型融合不能只用 accuracy 判断效果，必须同时报告 balanced accuracy、macro F1、collapse ratio 和 effective classes，以区分“真正恢复多类别诊断能力”和“只预测常见类别但被类别占比抬高”的两种情况。
