...
% 为缓解由异构诊断覆盖和冲突类别证据带来的聚合后预测坍缩，LAMP-Merge 首先重建一个类别条件诊断头。该模块不直接平均分类器参数，而是把每个客户端对某个类别是否具有监督证据显式写入融合过程，并在共享参考特征空间中聚合这些证据。
To mitigate the \textbf{post-aggregation prediction collapse} caused by heterogeneous diagnostic coverage and conflicting class evidence, LAMP-Merge first reconstructs a class-conditional diagnostic head. This module does not directly average classifier parameters; instead, it explicitly encodes whether each client has supervised evidence for a diagnosis class and aggregates such evidence in a shared reference feature space.

% 具体而言，令 phi_0 表示本地训练前由共享模型配置实例化得到的冻结参考骨干网络，T(.) 表示评估变换。对于每个诊断类别 c，客户端 i 计算参考空间类别原型 mu_i,c，其中 n_i,c=|D_i,c| 是类别支持数。若 n_i,c=0，则客户端对该类别上传零支持数，并且不贡献原型证据。
Specifically, let $\phi_0$ be the frozen reference backbone instantiated from the shared model configuration before local training, and let $T(\cdot)$ be the evaluation transform. For each diagnosis class $c$, client $i$ computes a reference-space class prototype
\begin{equation}
...
% 服务器随后将类别支持数转换为证据权重，其中 gamma=0.45 为默认证据指数。该权重使具有更多同类监督样本的客户端对该类别贡献更大，同时完全排除未观察到该类别的客户端。
The server converts support counts into evidence weights:
\begin{equation}
e_{i,c}=(n_{i,c}+1)^\gamma \mathbf{1}[n_{i,c}>0],
\end{equation}
where $\gamma=0.45$ is the default evidence exponent. This weight gives larger class-specific contribution to clients with more supervised samples of class $c$, while excluding clients that never observe this class. The normalized reliability of client $i$ for class $c$ is
\begin{equation}
\alpha_{i,c}=\frac{e_{i,c}}{\sum_{j=1}^{K} e_{j,c}}.
\end{equation}
% 全局诊断原型由各客户端原型按照类别可靠性加权得到。基于该原型，LAMP-Merge 合成余弦原型分类器，其中 s=20 为默认分类头缩放因子。由此，融合模型获得覆盖全局诊断空间的多类别判别方向。
The global diagnosis prototype is then obtained by class-wise reliability-weighted aggregation:
\begin{equation}
p_c=\sum_{i=1}^{K}\alpha_{i,c}\mu_{i,c}.
\end{equation}
Based on this prototype, LAMP-Merge synthesizes a cosine prototype classifier:
\begin{equation}
w_c=s\frac{p_c}{\|p_c\|_2},
\end{equation}
where $s=20$ is the default head scale. The resulting head provides multiclass diagnostic directions over the global label space.

\subsection{Long-tail Prevalence Calibration}

% 为缓解医学数据中真实长尾患病率不可忽略的问题，LAMP-Merge 在原型重建之后加入一个有界先验校准项。该模块的目标不是鼓励模型坍缩到多数类，而是在已经恢复多类别判别方向的基础上，保留由客户端统计反映的临床患病率。
To address the fact that the real long-tailed prevalence in medical data cannot be ignored, LAMP-Merge adds a bounded prior-calibration term after prototype reconstruction. The goal is not to encourage collapse toward majority classes; rather, it preserves the clinical prevalence reflected by client statistics after multiclass discriminative directions have been reconstructed.

% 每个客户端上传类别患病率计数 m_i,c。服务器据此估计全局类别先验 pi_c。该先验只来自类别级计数，而不是原始图像、逐样本特征或逐样本预测。
Each client uploads class-prevalence counts $m_{i,c}$. The server estimates the global class prior
\begin{equation}
...
% 为避免在非长尾场景中过度校准，服务器用 r=C max_c pi_c 度量主导类别不平衡。该模块仅在 r>tau 时激活，其中 tau=2.5 为默认值。
To avoid unnecessary calibration in non-long-tailed cases, the server measures dominant-class imbalance by
\begin{equation}
r=C\max_c \pi_c.
\end{equation}
The calibration module is activated only when $r>\tau$, where $\tau=2.5$ by default.

% 当该模块被激活时，LAMP-Merge 构造中心化对数先验偏置 b_c，其默认最大校准强度为 lambda=5.0。中心化操作避免所有类别 logits 被整体平移，使该项只改变类别相对先验。
When activated, LAMP-Merge constructs a centered log-prior bias
\begin{equation}
