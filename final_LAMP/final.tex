...
% LAMP-Merge 的最终分类函数整合了诊断原型重建项与长尾患病率校准项：
...
\end{equation}
% 其中 tau 用于判断是否激活长尾校准，lambda 用于控制校准强度。LAMP-Merge 的单次融合流程由上述得分函数给出。
where $\tau$ determines whether long-tail calibration is activated and $\lambda$ controls the calibration strength. The one-shot merging procedure of LAMP-Merge is summarized by the above scoring function.
