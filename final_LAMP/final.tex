...
% 医学人工智能越来越依赖多中心协作：单个医院通常难以覆盖稳健诊断部署所需的完整疾病谱、罕见病例和成像条件。然而，隐私法规、设备异质性和院内流程使原始影像集中化变得不现实。这推动了一种训练后协作范式：各医院在本地完成训练，并仅暴露有限信息以支持全局模型构建。
Medical artificial intelligence increasingly depends on \textbf{multicenter collaboration}: a single hospital rarely covers the full disease spectrum, rare cases, and acquisition conditions required for robust diagnostic deployment \cite{sheller2020federated,rieke2020future,kairouz2021advances}. However, privacy regulations, device heterogeneity, and institutional workflows make \textbf{centralizing raw images impractical} \cite{kaissis2020secure,bonawitz2019towards,abadi2016deep}. This motivates a post-training collaboration paradigm in which hospitals train locally and expose only limited information for global model construction.
...
% 这一现实约束导向单次异步模型融合。不同医院可能在不同时间完成训练，也可能无法像传统联邦或去中心化协议那样持续参与全局更新。因此，服务器必须在不访问原始医学图像的前提下，仅依赖每个客户端的一次上传构建可部署的全局诊断模型。
This practical constraint leads to \textbf{single-shot asynchronous model merging}. Hospitals may finish training at different times and may not remain available for repeated global updates, as required by conventional federated and decentralized protocols\cite{pmlr-v54-mcmahan17a,li2020fedprox,karimireddy2020scaffold,warnat-herresthal_swarm_2021}. The server must therefore construct one deployable diagnostic model from \textbf{exactly one upload per client}, without accessing raw medical images.

% 医学图像使该协议与标准检查点融合存在本质差异。医院之间的差异不仅来自设备域偏移，还来自诊断类别可及性：一个中心可能拥有大量常见病样本，却几乎没有罕见疾病；另一个中心则可能覆盖不同的诊断子集。因此，本地模型并非全局任务的独立副本，而是只对已见类别具有可靠证据的局部诊断专家。此外，医学标签通常呈长尾分布，多数类占优可能是具有临床意义的真实患病率，而不是单纯噪声。有效的融合方法必须恢复分散的类别证据，同时避免抹除真实患病率。
Medical imaging makes this protocol fundamentally different from standard checkpoint merging. Hospitals differ not only in device domains but also in \textbf{diagnostic-class availability} \cite{9557808,10.1371/journal.pmed.1002683,li2021fedbn}: one site may contain abundant common cases but few rare diseases, while another may cover a different subset. As a result, local models are not independent replicas of the global task; they are partial diagnostic experts with reliable evidence only for observed classes. In addition, medical labels are often \textbf{long-tailed}, so majority-class dominance may be clinically real rather than mere noise \cite{BUDA2018249,5597285}. A useful merge must therefore recover distributed class evidence without erasing meaningful prevalence.

% 为解决上述错配，本文提出 LAMP-Merge，一种长尾感知医学原型融合框架。与将融合视为全局参数插值不同，LAMP-Merge 将其重构为类别级诊断证据重建。每个客户端使用共享参考特征提取器计算类别原型和患病率计数，服务器据此重建全局原型分类器，并施加有界患病率校准。整个流程仅需一次上传，且不暴露原始图像、逐样本特征、logits 或预测。
To address this mismatch, we propose \textbf{LAMP-Merge}, a long-tail-aware medical prototype-merging framework. Instead of treating merging as global parameter interpolation, LAMP-Merge recasts it as \textbf{class-level diagnostic-evidence reconstruction}. Each client computes class prototypes and prevalence counts with a shared reference extractor, while the server reconstructs a global prototype classifier and applies bounded prevalence calibration. The procedure requires one upload and never exposes raw images, per-sample features, logits, or predictions.

% 本文贡献概括如下。第一，问题重审：我们形式化单次异步医学图像模型融合，其中独立训练的客户端仅提供一次上传用于全局模型构建。第二，失效诊断：我们指出诊断类别可及性异质和疾病患病率长尾是使通用融合方法容易发生预测坍缩的关键结构。第三，医学原型融合：我们提出 LAMP-Merge，通过客户端侧类别原型和患病率统计重建全局诊断分类器，同时不暴露原始医学图像。
Our contributions are summarized as follows:
\begin{itemize}
    \item \textbf{Problem Revisit.} We formalize single-shot asynchronous medical image model merging, where independently trained clients provide one upload for global model construction.
    \item \textbf{Failure Diagnosis.} We identify heterogeneous diagnostic-class availability and long-tailed disease prevalence as the structures that make general-purpose merging prone to prediction collapse.
    \item \textbf{Medical Prototype Merging.} We propose LAMP-Merge, which reconstructs a global diagnostic classifier from client-side class prototypes and prevalence statistics without exposing raw images.
\end{itemize}
...
% 在局部类别覆盖不完整时，参数级融合会把多个未坍缩的局部预测器转化为全局坍缩预测器。我们将这一行为概括为诊断证据错配：本地检查点包含类别特异证据，而通用融合将所有参数方向都视为全局可比较。由于医学患病率通常呈长尾分布，坍缩预测器仍可能在原始准确率上表现得具有竞争力。因此，有效的融合方法必须恢复罕见类别判别能力，同时不丢弃真实患病率。
Under partial local class coverage, parameter-level merging can turn several non-degenerate local predictors into a globally collapsed predictor. We summarize this behavior as a diagnostic-evidence mismatch: local checkpoints contain class-specific evidence, whereas generic merging treats all parameter directions as globally comparable. Because medical prevalence is often long-tailed, the collapsed predictor can still appear competitive in raw accuracy. A useful merger must therefore recover rare-class discrimination without discarding genuine prevalence.

% 为解决这一问题，LAMP-Merge 将融合目标从参数空间共识转向诊断空间证据重建。诊断原型重建在共享参考空间中聚合具有类别支持的证据，以恢复全局多类别方向。长尾患病率校准随后根据客户端侧计数施加有界先验偏置，在不替代这些判别方向的前提下保留具有临床意义的多数类患病率。该设计将判别证据重建与患病率校准解耦，使模型能够在不进行额外联邦训练、不暴露原始数据的条件下完成稳健的单次融合。
To address this issue, LAMP-Merge shifts the merge target from parameter-space consensus to diagnosis-space evidence reconstruction. Diagnostic prototype reconstruction aggregates class-supported evidence in a shared reference space to recover global multiclass directions. Long-tail prevalence calibration then applies a bounded prior bias estimated from client-side counts, preserving clinically meaningful majority prevalence without replacing those directions. This design separates \textbf{discriminative evidence reconstruction} from \textbf{prevalence calibration}, enabling robust one-shot merging without additional federated training or raw-data exposure.
...
% 假设存在 K 个医学客户端。客户端 i 持有私有标注数据集 D_i={(x,y)}，其来自覆盖 C 个诊断类别的类别偏斜医学分布。令 D_i,c 表示客户端 i 中属于诊断类别 c 的样本子集。在医学划分中，D_i,c 对许多类别可能为空。因此，客户端对已见诊断类别可以提供可靠证据，但对未见类别不具备监督支撑。类别无关的参数融合无法显式保留这种局部可靠性结构。
...
% 因此，LAMP-Merge 首先在共享参考特征空间中重建逐类别诊断头。令 phi_0 表示本地训练前由共享模型配置实例化得到的冻结参考骨干网络，T(.) 表示评估变换。对于每个诊断类别 c，客户端 i 计算参考空间类别原型 mu_i,c，其中 n_i,c=|D_i,c| 是类别支持数。若 n_i,c=0，则客户端对该类别上传零支持数，并且不贡献原型证据。
...
% 服务器将支持数转换为证据权重，其中 gamma=0.45 为默认证据指数。客户端 i 对类别 c 的归一化可靠性记为 alpha_i,c。随后，全局诊断原型 p_c 由各客户端原型按照可靠性加权得到。最后，LAMP-Merge 合成一个余弦原型分类器，其中 s=20 为默认分类头缩放因子。该模块的作用是把“某客户端对某类别是否有证据”显式写入融合过程，从而重建完整的全局多类别诊断头。
...
% 在重建诊断头之后，每个客户端额外上传类别患病率计数 m_i,c，用于刻画真实医学任务中的长尾患病率。服务器据此估计全局类别先验 pi_c，并用 r=C max_c pi_c 度量主导类别不平衡。该模块仅在 r>tau 时激活，其中 tau=2.5 为默认值。激活后，模型加入中心化对数先验偏置 b_c，其默认最大校准强度为 lambda=5.0。最终得分为 score_c(x)。该偏置是中心化且有界的，并附着在原型分类器上；它只校准患病率，不替代诊断原型重建得到的多类别判别方向。
