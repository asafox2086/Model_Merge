...
\hline \hline
\rowcolor{gray!10}
 & Best ref. & 80.20 & 85.18 & 72.38 & 71.12 & 80.31 & 79.02 \\
\rowcolor[HTML]{FFF9C4}
\multirow{-2}{*}{Collapse $\downarrow$} & \textbf{LAMP} & \textbf{19.58} & \textbf{68.05} & \textbf{20.72} & \textbf{24.36} & \textbf{20.45} & \textbf{30.63} \\
\hdashline
\rowcolor{gray!10}
 & Best ref. & 1.81 & 1.65 & 2.49 & 2.53 & 1.98 & 2.05 \\
\rowcolor[HTML]{FFF9C4}
\multirow{-2}{*}{Eff. classes $\uparrow$} & \textbf{LAMP} & \textbf{7.48} & \textbf{3.10} & \textbf{9.55} & \textbf{8.44} & \textbf{7.44} & \textbf{7.20} \\
\hdashline
\rowcolor{gray!10}
 & Best ref. & 73.48 & 46.03 & 75.42 & 73.85 & 72.73 & 69.44 \\
\rowcolor[HTML]{FFF9C4}
\multirow{-2}{*}{Pred-true TV $\downarrow$} & \textbf{LAMP} & \textbf{3.97} & \textbf{17.09} & \textbf{12.38} & \textbf{13.17} & \textbf{15.51} & \textbf{12.42} \\
\noalign{\hrule height 1.25pt}
\end{tabular}
}
\end{table}
...
Fig.~\ref{fig:hparam-analysis} evaluates the prototype-head scale $s$ and calibration strength $\lambda$ over 180 raw cells and 60 client-average cells. The fixed method uses $\gamma=0.55$, $s=18.75$, $\tau=2.5$, and $\lambda=4.25$; these values lie in broad stable regions, indicating that the gain is not produced by narrow hyperparameter tuning.

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/new/05_hyperparameter_sensitivity.pdf}
\caption{Full-scope hyperparameter sensitivity of DPR and LPC. Expanded vertical ranges show stable accuracy across broad intervals.}
...
The DPR and LPC sensitivity curves are computed from the complete 2026-07-19 sweep rather than from selected runs. DPR uses \(\gamma\in\{0.50,0.55,0.60\}\) and ten prototype-head scales \(s\in\{13.75,15.00,\ldots,25.00\}\). LPC uses \(\tau\in\{2.0,2.5,3.0\}\) and ten calibration strengths \(\lambda\in\{3.00,3.25,\ldots,5.25\}\). Thus, each module has a \(3\times10\) grid. At every grid point, we evaluate the five datasets, four backbone families, three client counts, and three Dirichlet skew levels, yielding 180 raw result cells and 60 client-average cells after averaging the three skew levels for each dataset--backbone--client-count combination.
