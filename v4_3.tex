...
To identify the necessary components of DPR, we retain LPC and replace its prototype semantics with classifier-head aggregation (CHA), global-feature mean (GFM), support-only synthetic head (SSH), or shuffled-label prototype (SLP). We also replace its client-support weighting with uniform client weight (UCW), binary support only (BSO), or global client-size weight (GCSW).
...
To evaluate LPC, we retain DPR and compare its empirical prior with the uniform prevalence prior (UPP), always-on calibration (AOC), and client-balanced prior (CBP) across the full experimental setting.
...
Fig.~\ref{fig:hparam-analysis} evaluates full-configuration DPR/LPC sensitivity; each marker averages 180 raw and 60 client-average cells.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/05_hyperparameter_sensitivity.png}
\caption{Full-configuration DPR/LPC sensitivity. Each point averages 60 client-average cells.}
...
We study one-shot asynchronous medical model merging without raw-image sharing or federated rounds. Generic merges can suffer aggregation-induced prediction collapse under incomplete local diagnostic coverage, while raw ACC may conceal this failure on long-tailed medical data. Across full-scope comparisons, ablations, and prediction-distribution diagnostics, DPR restores class-level diagnostic directions and LPC retains clinically meaningful prevalence without reintroducing collapse. These results establish a concise mechanism: support-weighted reconstruction recovers discriminative directions for locally absent classes, while bounded calibration preserves meaningful imbalance. By using only uploadable class-level statistics, LAMP-Merge offers a practical and interpretable foundation for broader multicenter medical imaging tasks.
