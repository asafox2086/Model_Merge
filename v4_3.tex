...
Fig.~\ref{fig:hparam-analysis} evaluates DPR and LPC sensitivities under the full LAMP-Merge configuration. Each curve point uses 180 raw cells and 60 client-average cells. The fixed operating point uses $\gamma=0.55$, $s=18.75$, $\tau=2.5$, and $\lambda=4.25$; the sweeps assess deviations around these values while holding the other module fixed.

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/new/05_hyperparameter_sensitivity.pdf}
\caption{DPR and LPC sensitivity under the full LAMP-Merge configuration. Each line varies one module while fixing the other at its selected operating point. Each point averages 60 client-average cells.}
...
The sensitivity figure uses full-configuration sweeps only. The DPR grid uses \(\gamma\in\{0.50,0.55,0.60\}\) and ten prototype-head scales \(s\in\{13.75,15.00,\ldots,25.00\}\), while holding \(\tau=2.5\) and \(\lambda=4.25\) fixed. The LPC grid uses \(\tau\in\{2.0,2.5,3.0\}\) and ten calibration strengths \(\lambda\in\{3.00,3.25,\ldots,5.25\}\), while holding \(\gamma=0.55\) and \(s=18.75\) fixed. At every grid point, we evaluate the five datasets, four backbone families, three client counts, and three Dirichlet skew levels, yielding 180 raw result cells and 60 client-average cells after averaging the three skew levels for each dataset--backbone--client-count combination.
