...
\caption{DPR/LPC sensitivity on \textbf{the full setting}.}
\label{fig:hparam-analysis}
\end{figure}

The DPR curves are stable around moderate prototype-head scales, and the selected \(s=18.75\) lies in this broad high-performing region for all displayed \(\gamma\) values. LPC changes gradually with \(\lambda\), while the threshold curves remain close around the selected \(\tau=2.5\). These trends show that the selected operating point is representative rather than an isolated peak.
