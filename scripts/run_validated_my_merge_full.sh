...
ABLATIONS="${ABLATIONS:-full no_client_information no_fusion_selection}"
...
for ablation in "${ABLATION_ARRAY[@]}"; do
  ...
  env \
    ...
    MY_MERGE_FEATURE_SUMMARY_ROOT="${MY_MERGE_FEATURE_SUMMARY_ROOT}" \
    MY_MERGE_REQUIRE_FEATURE_SUMMARY="${MY_MERGE_REQUIRE_FEATURE_SUMMARY}" \
    MY_MERGE_EXPORT_DIAGNOSTICS="${MY_MERGE_EXPORT_DIAGNOSTICS}" \
    MY_MERGE_DOMAIN_CONTROL="${MY_MERGE_DOMAIN_CONTROL}" \
