...
def plot_embedding(path, coordinates, labels, title):
    import matplotlib
    ...
    axis.set_xticks([])
    axis.set_yticks([])
    axis.legend(title="True label", loc="best", markerscale=1.5, frameon=True)
...
def main():
    ...
    summaries = {}
    domains = [
        ("medical_dermamnist", medical_meta, args.medical_hub_dir, args.medical_data_root),
        ("natural_cifar", natural_meta, args.natural_hub_dir, args.natural_data_root),
    ]
    for domain, meta, hub_dir, data_root in domains:
        ...
        plot_embedding(
            args.output_dir / f"{domain}_resnet_k3_avg_feature_tsne.png",
            coordinates,
            labels,
            f"{domain} / ResNet / K=3 / {args.merge_weighting} AVG features",
        )
        rows.extend(
            {
                "domain": domain,
