...
from style import darken_color, polish_axes, setup_style
...
def save_png_pdf(figure, output_base: Path, dpi: int = 350) -> None:
    output_base.parent.mkdir(parents=True, exist_ok=True)
    figure.savefig(str(output_base) + ".png", dpi=dpi, bbox_inches="tight")
...
def main() -> None:
    ...
    if args.layout == "overlay":
        ...
        for suffix, predicted, true, title, color, zero_summary in (
            ...
            axis.legend(loc="upper left", frameon=False)
            figure.tight_layout()
            save_png_pdf(figure, args.output_base.with_name(args.output_base.name + f"_{suffix}"), dpi=350)
            plt.close(figure)
        return
    else:
    ...
    save_png_pdf(figure, args.output_base, dpi=350)
    plt.close(figure)
