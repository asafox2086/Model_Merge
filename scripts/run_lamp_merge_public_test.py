#!/usr/bin/env python3
"""Run external public-data evaluation for LAMP-Merge.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run LAMP-Merge on an external public evaluation split.")
    ...
    parser.add_argument("--source-config", type=Path, default=ROOT / "outputs" / "lamp_merge_full" / "reports" / "batch_config.json")
    parser.add_argument("--source-eval-summary", type=Path, default=ROOT / "outputs" / "lamp_merge_full" / "reports" / "eval_summary.csv")
    ...
    parser.add_argument("--methods", nargs="*", default=["lamp_merge"])
        ...
        lines.extend(
            [
                ...
                md_table(
                    ...
                    [["lamp_merge", len(deltas), f"{mean(deltas):+.4f}", f"{min(deltas):+.4f}", f"{max(deltas):+.4f}"]],
                ),
            ]
        )
