...
def build_merge_cfg(row: dict[str, str], method: str, args: argparse.Namespace) -> dict[str, object]:
...
def base_result_row(row: dict[str, str], source: str, method: str, client_id: str, args: argparse.Namespace) -> OrderedDict[str, object]:
    return OrderedDict(
        {
            "status": "OK",
            "error": "",
...
def evaluate_method_row(
    ...
    result = base_result_row(row, "merge", method, "", args)
    key = result_key(result)
    if args.resume and key in existing:
        return
    start = time.time()
    try:
        cfg = build_merge_cfg(row, method, args)
...
def main() -> None:
    ...
    total = len(manifest) * len(args.methods) + (sum(int(row["num_clients"]) for row in manifest) if args.include_clients else 0)
    current = 0
    print(
        f"Selected cases={len(manifest)} methods={len(args.methods)} include_clients={args.include_clients} "
        f"total_evals={total}"
    )
    ...
    for row in manifest:
        ...
        for method in args.methods:
            before = len(existing)
            evaluate_method_row(row, method, args, fields, existing)
            current += 1
            status = "skip" if len(existing) == before else "done"
            print(f"[{current}/{total}] {status} {method} {label}")
