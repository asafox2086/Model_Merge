...
This document is the self-contained supplement to the main paper. It is organized as follows.
Section~\ref{app:organ-s-results} reports the Organ-S state-of-the-art comparison that is omitted from the enlarged main-text tables.
Section~\ref{app:datasets} gives a detailed, per-class description of the five MedMNIST~v2 benchmarks used in all experiments, including the exact training, validation, and test split sizes and representative samples.
Section~\ref{app:ablation-configurations} specifies, for every internal ablation reported in the main paper, the exact quantity that is substituted in the LAMP-Merge pipeline, together with its measured effect; this makes each ablation label reproducible from the equations alone.
Section~\ref{app:theory} develops the theoretical analysis behind the two modules: it states the estimation assumptions explicitly and proves (i) a bias--variance bound for the reconstructed prototypes, (ii) a sufficient condition under which prototype reconstruction preserves pairwise class margins, and (iii) the boundedness and gauge-invariance of the prevalence correction.
Section~\ref{app:metric-geometry} defines the prediction-collapse metrics, and Section~\ref{app:repro} records the hyperparameter grid used for the sensitivity study.
Notation follows the main paper: $K$ clients index $i$, $C$ diagnosis classes index $c$, $n_{i,c}$ is client $i$'s support for class $c$, $m_{i,c}$ its prevalence count, and $\phi_0(T(\cdot))$ the frozen reference encoder applied to the evaluation transform $T$.

\section{Organ-S Results Omitted from the Main Tables}
\label{app:organ-s-results}

The main paper enlarges the state-of-the-art comparison tables by reporting four representative benchmarks in the two-column body. For completeness, Tables~\ref{tab:appendix-organs-swin}--\ref{tab:appendix-organs-vit} report the omitted Organ-S client-average results under the same protocol. Each cell gives ACC / Macro-F1 in percentage points.
...
The experiments use five MedMNIST~v2 benchmarks~\citep{yang2023medmnist}. They were selected to cover microscopy, dermoscopy, CT, and ultrasound while varying the number of diagnostic classes and the prevalence profile. The per-class tables in this section (Tables~\ref{tab:appendix-blood-classes}--\ref{tab:appendix-ultrasound-classes}) report the exact training, validation, and test split sizes used by the experiments; the descriptions below clarify the recognition problem represented by each benchmark.
...
\paragraph{Ultrasound.} UltrasoundMNIST is an eight-class ultrasound-structure benchmark. Speckle noise, low contrast, and view-dependent boundaries make it particularly suitable for the prediction-collapse diagnostics and the per-class gap visualization reported in the main paper.
...
\begin{assumption}[Within-client sampling]
\label{as:sampling}
Fix a class $c$. For every client $i$ with support $n_{i,c}>0$, the reference features $\{\phi_0(T(x)):(x,y)\in D_{i},\,y=c\}$ are i.i.d.\ draws from a client-conditional distribution with mean $\mu_{i,c}^{\star}:=\mathrm{E}[\phi_0(T(x))\mid y=c,\text{client }i]$ and covariance $\Sigma_{i,c}$ satisfying $\operatorname{tr}(\Sigma_{i,c})\le\sigma_c^{2}$. Features drawn by different clients are mutually independent.
\end{assumption}

\begin{assumption}[Prevalence positivity]
\label{as:prevalence}
...
\begin{proposition}[Bias--variance bound]
\label{prop:bound}
...
\end{equation}
and for an average of $n_{i,c}$ i.i.d.\ vectors $\mathrm{E}\|\mu_{i,c}-\mu_{i,c}^{\star}\|_2^{2}=\operatorname{tr}(\Sigma_{i,c})/n_{i,c}\le\sigma_c^{2}/n_{i,c}$. Terms with $n_{i,c}=0$ carry $\alpha_{i,c}=0$ and drop out, yielding \eqref{eq:prototype-bound}.
\end{proof}
...
\begin{proposition}[Margin preservation]
\label{prop:margin}
...
\begin{proposition}[Zero-sum, invariance, and boundedness]
\label{prop:lpc}
...
Let \(y_n\) and \(\hat{y}_n\) denote the true and predicted labels of sample \(n\), respectively. Let \(q_c=N^{-1}\sum_n\mathbf{1}[\hat{y}_n=c]\) be the predicted class distribution and \(\bar{q}_c=N^{-1}\sum_n\mathbf{1}[y_n=c]\) the true distribution, as in the main-text collapse diagnostics. The collapse ratio \(\rho=\max_c q_c\) measures the largest predicted-class share, \(C_{\mathrm{eff}}=\exp(-\sum_c q_c\log q_c)\) measures the effective number of classes used by the predictor, and \(\mathrm{TV}(q,\bar{q})=\frac{1}{2}\sum_c|q_c-\bar{q}_c|\) measures distributional disagreement with the labels. Lower \(\rho\) and TV and higher \(C_{\mathrm{eff}}\) are preferred. The absolute per-class gaps visualized in the main paper are \(|q_c-\bar{q}_c|\) in percentage points and expose which classes contribute to TV.
