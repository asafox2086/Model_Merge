...
\paragraph{Use of a non-public ultrasound subset.} ChaoShengMNIST is included to evaluate whether one-shot medical model merging remains reliable under an ultrasound-specific imaging regime. The dataset contains 5,531 ultrasound images from eight anonymized diagnostic categories, with 3,869 training images, 549 validation images, and 1,113 test images; the exact per-class split sizes and representative samples are reported in Table~\ref{tab:appendix-ultrasound-classes}. We keep the clinical labels anonymized and do not redistribute the raw images because the subset is governed by private medical-data access constraints. Its role in this paper is limited to evaluation: it provides an additional modality with speckle noise, operator-dependent views, low soft-tissue contrast, and class-dependent prevalence, all of which differ from the public microscopy, dermoscopy, and CT benchmarks.

Publicly available alternatives are not scientifically sufficient substitutes for this experiment. BloodMNIST, DermaMNIST, OrganCMNIST, and OrganSMNIST are valuable public MedMNIST~v2 benchmarks, but they do not provide an ultrasound classification setting with the same imaging physics, acquisition variability, and eight-class diagnostic structure. Replacing ChaoShengMNIST with another public non-ultrasound benchmark would test only whether LAMP-Merge transfers across more public datasets; it would not test whether the method remains stable under ultrasound-specific domain shift. Similarly, public ultrasound resources often target different tasks, such as segmentation, plane recognition, or binary screening, and therefore do not match the single-label multi-class merging protocol used here. Thus, ChaoShengMNIST is used as a complementary private ultrasound stress test rather than as a substitute for the public benchmarks.
...
\paragraph{LPC summary.} Every LPC control is below the 62.21\% LAMP-Merge mean. UPP and CBP show that both prevalence information and its sample-size weighting matter; AOC shows that the activation criterion is a targeted safeguard rather than the primary source of the gain. \textbf{Hence LPC calibrates DPR's restored class directions instead of replacing them with a prior.}
