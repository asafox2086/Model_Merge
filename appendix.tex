...
Section~\ref{app:organ-s-results} reports the Organ-S comparison table in full. Section~\ref{app:datasets} reports dataset provenance and per-class statistics, distinguishing the four public MedMNIST~v2 benchmarks from the private ChaoShengMNIST ultrasound subset.
Section~\ref{app:algorithm-figures} explains the algorithmic flow and the figures used by the main paper and appendix.
...
Section~\ref{app:theory} gives the theoretical analysis behind the two modules, including a bias and variance bound for diagnostic prototype reconstruction, a margin-preservation condition, and the boundedness of the prevalence correction.
...
\section{Algorithm and Figure Explanations}
\label{app:algorithm-figures}

This section expands the algorithm and visual material used in the main paper. The goal is to make each symbol, processing step, and figure traceable to the same class-level merge protocol.

\subsection{LAMP-Merge Algorithm}

For each client \(i\) and diagnosis class \(c\), the client first computes the class support and reference-space prototype:
\begin{equation}
n_{i,c}=|D_{i,c}|.
\label{eq:app-alg-support}
\end{equation}
\begin{equation}
\mu_{i,c}=
\frac{1}{n_{i,c}}
\sum_{(x,y)\in D_i}
\mathbf{1}[y=c]\phi_0(T(x)).
\label{eq:app-alg-prototype}
\end{equation}
The prototype in Eq.~\ref{eq:app-alg-prototype} is computed only when \(n_{i,c}>0\). A client with no sample from class \(c\) uploads zero support for that class and does not contribute to its prototype aggregation.

The server converts class support into a reliability weight:
\begin{equation}
e_{i,c}=(n_{i,c}+1)^\gamma\mathbf{1}[n_{i,c}>0].
\label{eq:app-alg-evidence}
\end{equation}
The normalized aggregation coefficient is
\begin{equation}
\alpha_{i,c}=\frac{e_{i,c}}{\sum_{j=1}^{K}e_{j,c}}.
\label{eq:app-alg-alpha}
\end{equation}
The global class prototype and the induced classifier weight are
\begin{equation}
p_c=\sum_{i=1}^{K}\alpha_{i,c}\mu_{i,c}.
\label{eq:app-alg-global-prototype}
\end{equation}
\begin{equation}
w_c=s\frac{p_c}{\|p_c\|_2}.
\label{eq:app-alg-head}
\end{equation}
Thus, Diagnostic Prototype Reconstruction builds the classifier head from class-conditioned evidence in the shared reference space.

Long-tail Prevalence Calibration uses client-reported prevalence counts to estimate the global class prior:
\begin{equation}
\pi_c=
\frac{\sum_{i=1}^{K}m_{i,c}}
{\sum_{k=1}^{C}\sum_{i=1}^{K}m_{i,k}}.
\label{eq:app-alg-prior}
\end{equation}
The dominant-class imbalance is
\begin{equation}
r=C\max_c\pi_c.
\label{eq:app-alg-imbalance}
\end{equation}
When \(r>\tau\), the centered prior bias is
\begin{equation}
b_c=
\lambda
\left(
...
\right).
\label{eq:app-alg-bias}
\end{equation}
When \(r\le\tau\), \(b_c=0\). The final class score is
\begin{equation}
\ell_c(x)=w_c^\top\phi_0(T(x))+\mathbf{1}[r>\tau]b_c.
\label{eq:app-alg-score}
\end{equation}

\begin{algorithm}[!t]
\caption{LAMP-Merge in One Upload}
\label{alg:app-lamp}
\begin{algorithmic}[1]
\REQUIRE Clients \(\{D_i\}_{i=1}^{K}\), reference encoder \(\phi_0\), transform \(T\), hyperparameters \(\gamma,s,\tau,\lambda\)
\ENSURE Merged model \(\theta_m\)
\STATE Each client computes class support by Eq.~\ref{eq:app-alg-support}
\STATE Each client computes class prototypes by Eq.~\ref{eq:app-alg-prototype}
\STATE Each client uploads only class-level statistics
\STATE The server computes reliability weights by Eq.~\ref{eq:app-alg-evidence}
\STATE The server computes aggregation coefficients by Eq.~\ref{eq:app-alg-alpha}
\STATE The server reconstructs prototypes and classifier weights by Eqs.~\ref{eq:app-alg-global-prototype} and~\ref{eq:app-alg-head}
\STATE The server estimates prevalence and imbalance by Eqs.~\ref{eq:app-alg-prior} and~\ref{eq:app-alg-imbalance}
\STATE The server computes the calibration bias by Eq.~\ref{eq:app-alg-bias}
\STATE The server evaluates class scores by Eq.~\ref{eq:app-alg-score}
\RETURN \(\theta_m=(\phi_0,\{w_c,b_c\}_{c=1}^{C})\)
\end{algorithmic}
\end{algorithm}

Algorithm~\ref{alg:app-lamp} separates the hospital-side computation from the server-side merge. Hospitals only upload class-level statistics. Raw images, per-sample features, and per-sample predictions remain local. The server constructs a single global diagnostic head in one pass.

\subsection{Figure Guide}

The motivation figure in the main paper illustrates the structural difference between natural-image and medical-image model merging. It shows that medical clients can cover different diagnosis subsets under long-tailed prevalence, so a class-agnostic merge can mix supported evidence with absent-class directions.

The controlled domain-comparison figure uses matched class imbalance and matched client assignments for CIFAR-100 and DermaMNIST. It demonstrates that the same RegMean merge preserves multiclass predictions on the natural-image control while concentrating predictions on one DermaMNIST class. This figure supports the claim that medical diagnostic structure can interact with class-agnostic merging in a harmful way.

The method overview figure summarizes the complete LAMP-Merge pipeline. The left side corresponds to local computation of \(n_{i,c}\), \(m_{i,c}\), and \(\mu_{i,c}\). The server side corresponds to Eqs.~\ref{eq:app-alg-evidence} to~\ref{eq:app-alg-score}. Diagnostic Prototype Reconstruction supplies the class directions \(w_c\). Long-tail Prevalence Calibration supplies the bounded bias \(b_c\).

The dataset sample tables in Section~\ref{app:datasets} provide visual examples and exact split sizes for every class. They are included to show that the experiments cover microscopy, dermoscopy, CT, and ultrasound, and that the class statistics used by LAMP-Merge are directly tied to the benchmark files.

The module-ablation figure and internal-ablation tables test whether the two components and their internal variables are necessary. The prediction-distribution figures and collapse-diagnostic tables test whether performance gains coincide with reduced single-class prediction collapse. The hyperparameter figure records full-scope sensitivity for \(\gamma\), \(s\), \(\tau\), and \(\lambda\), showing that the reported operating point is not selected from an isolated run.

\section{Organ-S Supplementary Results}
\label{app:organ-s-results}

The main paper emphasizes the representative benchmarks in the two-column body. For completeness, Tables~\ref{tab:appendix-organs-swin} to~\ref{tab:appendix-organs-vit} report the Organ-S client-average results under the same protocol. Each cell gives ACC / Macro-F1 in percentage points. \textbf{Across all four backbones, LAMP-Merge remains the strongest method on Organ-S, showing that the reported conclusions are not an artifact of excluding this CT plane from a compact table layout.}
...
\paragraph{ChaoShengMNIST.} ChaoShengMNIST is a private eight-class ultrasound image subset. The source retains anonymized class identifiers, so we report the labels as Class 0 to Class 7 and do not assign unverified clinical names. Compared with microscopy and CT, ultrasound images contain stronger speckle noise, lower contrast, and view-dependent boundaries. This makes ChaoShengMNIST a useful stress test for the prediction-collapse diagnostics and the per-class distribution-gap visualization reported in the main paper.

\newcommand{\classsample}[2]{\raisebox{-0.12cm}{\includegraphics[width=1.02cm,height=0.58cm,keepaspectratio]{figures/appendix_sample_#1_#2.pdf}}}
...
\rowcolor{gray!10} & Left femur & 619 & 102 & 431 & 1,152 & \classsample{organ_c}{1} \\
 & Right femur & 595 & 96 & 421 & 1,112 & \classsample{organ_c}{2} \\
\rowcolor{gray!10} & Heart & 600 & 202 & 421 & 1,223 & \classsample{organ_c}{3} \\
 & Left kidney & 1,088 & 132 & 727 & 1,947 & \classsample{organ_c}{4} \\
\rowcolor{gray!10} & Right kidney & 1,170 & 157 & 735 & 2,062 & \classsample{organ_c}{5} \\
 & Liver & 2,986 & 429 & 1,835 & 5,250 & \classsample{organ_c}{6} \\
\rowcolor{gray!10} & Left lung & 1,002 & 347 & 549 & 1,898 & \classsample{organ_c}{7} \\
 & Right lung & 1,022 & 352 & 557 & 1,931 & \classsample{organ_c}{8} \\
...
\rowcolor{gray!10} & Left femur & 630 & 104 & 439 & 1,173 & \classsample{organ_s}{1} \\
 & Right femur & 614 & 95 & 445 & 1,154 & \classsample{organ_s}{2} \\
\rowcolor{gray!10} & Heart & 721 & 246 & 510 & 1,477 & \classsample{organ_s}{3} \\
 & Left kidney & 1,132 & 140 & 704 & 1,976 & \classsample{organ_s}{4} \\
\rowcolor{gray!10} & Right kidney & 1,119 & 159 & 693 & 1,971 & \classsample{organ_s}{5} \\
 & Liver & 3,464 & 491 & 2,078 & 6,033 & \classsample{organ_s}{6} \\
\rowcolor{gray!10} & Left lung & 741 & 261 & 397 & 1,399 & \classsample{organ_s}{7} \\
 & Right lung & 803 & 275 & 439 & 1,517 & \classsample{organ_s}{8} \\
...
\textbf{The main paper uses two empirical observations to motivate the two components of LAMP-Merge, and the two observations are measured with different objects.} The first is a \textbf{model-side} observation: after generic model merging, the prediction distribution can collapse to a small number of classes. The second is a \textbf{data-side} observation: medical image datasets themselves can be long-tailed, so a prevalence signal should not be removed indiscriminately. This separation avoids treating long-tail prevalence as merely another name for model collapse.
...
\end{equation}
\textbf{Prediction collapse occurs when $q_c$ concentrates on one or a few classes although the task contains a larger diagnosis space.} We quantify this phenomenon with the collapse ratio
\begin{equation}
\rho=\max_c q_c,
...
\label{eq:app-tv}
\end{equation}
The true test-set class distribution is
\begin{equation}
\bar{q}_c=\frac{1}{N}\sum_n\mathbf{1}[y_n=c].
\end{equation}
A collapsed merger has large \(\rho\), small \(C_{\mathrm{eff}}\), and large TV. This observation motivates Diagnostic Prototype Reconstruction: if generic parameter-space merging loses class-discriminative directions, the server should reconstruct the diagnostic head from class-level evidence.

\subsection{Long-Tailed Medical Prevalence}

\textbf{The second observation concerns the data distribution itself.} For a dataset split with class counts $N_c$, the empirical prevalence is
\begin{equation}
...
\paragraph{DPR summary.} The semantic controls cause large reductions from 35.71 to 51.32 points, and the weighting controls cause smaller but consistent reductions from 2.82 to 7.03 points. \textbf{Together, these results support the two parts of Eq.~\ref{eq:appendix-dpr}: class-conditioned shared-space evidence provides the discriminative direction, and class-specific support determines how reliably each client contributes to that direction.}
...
\paragraph{Uniform prevalence prior (UPP).} UPP replaces the empirical prior in Eq.~\ref{eq:appendix-lpc} by the uniform prior:
\begin{equation}
\pi_c\longrightarrow\frac{1}{C}.
\end{equation}
It tests whether calibration should ignore the observed class distribution. Mean ACC is 58.91\%, 3.30 points below LAMP-Merge, demonstrating that the empirical prior is useful for calibrated prevalence preservation.
...
\paragraph{LPC summary.} Every LPC control is below the 62.21\% LAMP-Merge mean. UPP and CBP show that both prevalence information and its sample-size weighting matter; AOC shows that the activation criterion is a targeted safeguard. \textbf{Hence LPC calibrates DPR's restored class directions while preserving the prototype-based classifier.}

\section{Theoretical Analysis}
\label{app:theory}

This section makes precise the two claims that motivate LAMP-Merge: support-aware prototype aggregation controls the estimation error of the reconstructed class directions (Sec.~\ref{app:theory-dpr}), and the centered, gated log-prior of LPC is a \emph{bounded} correction that adjusts those directions (Sec.~\ref{app:theory-lpc}). We work throughout in the shared reference space induced by the frozen encoder:
\begin{equation}
\phi_0(T(\cdot)).
\end{equation}
All client prototypes therefore live in a common coordinate system and can be compared directly.
...
\paragraph{Proposition 1 (Bias and variance bound).}
\label{prop:bound}
...
The cosine head used by LAMP-Merge is
\begin{equation}
w_c=s\frac{p_c}{\|p_c\|_2}.
\end{equation}
It scores in the direction of \(p_c\), so Proposition~\ref{prop:margin} is the relevant guarantee: whenever DPR drives the per-class error below half the population margin in the metric of \eqref{eq:margin-condition}, the reconstructed head reproduces the population ranking. Proposition~\ref{prop:bound} shows that this error \emph{can} be made small by class-specific support weighting. Proposition~\ref{prop:margin} shows why doing so matters. The four semantic controls break the antecedent: \textbf{CHA} and \textbf{GFM} replace \(\mu_{i,c}\) by quantities whose expectation is not \(\mu_c^{\star}\), \textbf{SSH} discards \(\mu_{i,c}\) for a data-independent vector, and \textbf{SLP} permutes prototypes across labels. Each control destroys the margin the proposition relies on, consistent with their large losses from 35.71 to 51.32 points.
...
\ell_c(x)>\ell_d(x).
\end{equation}
The correction therefore acts only on near-ties, tipping them toward the more prevalent class, and cannot override a confident reconstructed direction. The gate is
\begin{equation}
\mathbf{1}[r>\tau],
\qquad
r=C\max_c\pi_c.
\end{equation}
It switches the correction off when the configuration is close to balanced. This is the formal counterpart of the empirical finding that a prevalence-only mechanism cannot recover the semantic controls. By (i) and (ii), the bias contributes no class-discriminative direction of its own. By (iii), its influence is bounded, so it can calibrate the directions DPR supplies without substituting for them.
