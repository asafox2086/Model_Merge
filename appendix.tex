...
Section~\ref{app:algorithm} provides the algorithmic form of LAMP-Merge and clarifies the client-side and server-side operations used to instantiate the method.
...
Algorithm~\ref{app:alg:lamp} gives the procedural form of LAMP-Merge. The purpose of this appendix algorithm is not to introduce an additional method beyond the main text, but to make explicit the execution order, the information boundary between hospitals and the server, and the exact quantities that are uploaded once by each client. The notation follows the main paper: \(D_i\) is the local dataset of client \(i\), \(D_{i,c}\) is the subset with diagnosis label \(c\), \(\phi_0\) is the frozen reference encoder, and \(T\) is the evaluation transform applied before feature extraction.

The algorithm is divided into two stages. Lines~1--6 describe the hospital-side computation. Each client computes class-level prototypes \(\mu_{i,c}\), support counts \(n_{i,c}\), and prevalence counts \(m_{i,c}\) using its own local data. Only these class-level statistics are uploaded, while raw images and patient-level samples remain local. Lines~7--20 describe the single-pass server-side merge. The server first performs Diagnostic Prototype Reconstruction by assigning support-aware weights \(\alpha_{i,c}\) and reconstructing one global prototype \(p_c\) for each diagnosis class. It then converts \(p_c\) into the classifier weight \(w_c\) and applies Long-tail Prevalence Calibration through the gated centered log-prior \(b_c\). The returned model is therefore a single diagnostic classifier \(\theta_m=(\phi_0,\{w_c,b_c\}_{c=1}^{C})\), with a frozen shared encoder and a reconstructed class-aware head.
...
This procedural view also clarifies the privacy and communication setting used in all experiments. The client upload in Line~6 consists of aggregated class-level objects \(\{(\mu_{i,c},n_{i,c},m_{i,c})\}_{c=1}^{C}\); it does not include raw images, sample identities, gradients, or iterative feedback from the server. The server-side computation is deterministic once these statistics and the hyperparameters \(\gamma,s,\tau,\lambda\) are fixed, which makes the reported merge reproducible from the same client statistics without running additional local training.
